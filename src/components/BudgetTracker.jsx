  ...
  const CommentButton = ({ rowId, columnId, rowName, columnName }) => {
    ...
    return (
        <button 
            ...
            className="absolute top-1/2 -translate-y-1/2 right-0 p-0.5 text-gray-400 hover:text-blue-600 opacity-0 group-hover/subcell:opacity-100 transition-opacity z-10"
            title="Commentaires"
        >
            <MessageSquare className="w-3 h-3" />
            {commentsForCell.length > 0 && (
                ...
                <td className="px-2 py-2">
                  {numVisibleCols > 0 && (
                    <div className="flex gap-2 justify-around text-sm font-bold">
                        <div className="relative group/subcell flex-1 text-center text-text-primary">
                        ...
                        <div className="relative group/subcell flex-1 text-center">
                            <button onClick={(e) => { e.stopPropagation(); if (totals.actual !== 0) handleActualClick({ type, period }); }} disabled={totals.actual === 0} className="text-text-primary hover:underline disabled:cursor-not-allowed disabled:opacity-60">
                          ...
                          <td className="px-2 py-1">
                            {numVisibleCols > 0 && (
                              <div className="flex gap-2 justify-around text-xs">
                                ...
                                <div className={`relative group/subcell flex-1 text-center font-medium ${getResteColor(reste, isEntree)}`}>
                                    {formatCurrency(reste, currencySettings)}
                      ...
                      return (
                          <React.Fragment key={periodIndex}>
                              <td className="px-2 py-2">
                                  {numVisibleCols > 0 && (
                                      <div className="flex gap-2 justify-around text-sm font-bold">
                                          <div className={`relative group/subcell flex-1 text-center ${netBudget < 0 ? 'text-red-600' : 'text-text-primary'}`}>
                                          ...
                                          <div className={`relative group/subcell flex-1 text-center ${getResteColor(netReste, true)}`}>
