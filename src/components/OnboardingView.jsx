...
console.log("🚀 OnboardingView: Chargement du composant");
...
const OnboardingView = () => {
  ...
  console.log("📊 OnboardingView: projects count=", projects.length, "hasExistingProjects=", hasExistingProjects);
  ...
  console.log("📋 OnboardingView: step=", step, "currentStepInfo=", currentStepInfo);

  const handleNext = () => {
    console.log("➡️ OnboardingView: Clic sur Suivant, step actuel=", step);
    
    if (step === 0 && !data.projectName.trim()) {
      console.log("❌ OnboardingView: Nom de projet requis");
      dispatch({ type: 'ADD_TOAST', payload: { message: "Le nom du projet est obligatoire.", type: 'error' } });
      return;
    }
    
    if (step === 1 && data.templateId === 'blank') {
      console.log("⚡ OnboardingView: Template vierge, saut vers l'étape finale");
    ...
    if (step < steps.length - 1) {
      setDirection(1);
      setStep(step + 1);
      console.log("✅ OnboardingView: Passage à l'étape", step + 1);
    }
  };

  const handleBack = () => {
    console.log("⬅️ OnboardingView: Clic sur Précédent, step actuel=", step);
    
    if (step === 3 && data.templateId === 'blank') {
      setDirection(-1);
      setStep(1);
      console.log("↩️ OnboardingView: Retour à la sélection de template");
      return;
    }
    if (step > 0) {
      setDirection(-1);
      setStep(step - 1);
      console.log("✅ OnboardingView: Retour à l'étape", step - 1);
    }
  };
  
  const handleCancel = () => {
    console.log("❌ OnboardingView: Annulation");
    dispatch({ type: 'CANCEL_ONBOARDING' });
  };

  const handleFinish = async () => {
    console.log("🎯 OnboardingView: Début de la création du projet", data);
    ...
    try {
      ...
      console.log("📤 OnboardingView: Envoi de la requête API", payload);
      const response = await api.post('/projects', payload);
      
      console.log("✅ OnboardingView: Réponse API", response.data);

      if (response.data.success) {
        console.log("🎉 OnboardingView: Projet créé avec succès");
        ...
        dispatch({
          ...
          payload: {
            ...
            finalCashAccounts: response.data.cash_accounts || [],
            newAllEntries: response.data.entries || [],
            newAllActuals: response.data.actuals || [],
            newTiers: response.data.tiers || [],
            newLoans: response.data.loans || [],
            newCategories: response.data.categories || null
          }
        });
        ...
        dispatch({ type: 'CANCEL_ONBOARDING' });
        console.log("➡️ OnboardingView: Navigation vers /app/dashboard");
      ...
      console.error("❌ OnboardingView: Erreur création projet:", error);
      dispatch({
        type: 'ADD_TOAST',
        payload: { 
      ...
      setIsLoading(false);
      console.log("🏁 OnboardingView: Fin du processus de création");
    }
  };
  ...
  console.log("📋 OnboardingView: Templates - official:", allOfficialTemplates.length, "community:", communityTemplates.length, "my:", myTemplates.length, "filtered:", filteredTemplates.length);
  ...
  const renderStepContent = () => {
    console.log("🎨 OnboardingView: Rendu de l'étape", currentStepInfo.id);
            ...
            <div className="space-y-6 max-w-md mx-auto">
              <div>
                ...
                <input 
                  ...
                  className="w-full text-lg p-2 border-b-2 focus:border-blue-500 outline-none transition bg-transparent" 
                  autoFocus 
                  required 
                />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full text-lg p-2 border-b-2 focus:border-blue-500 outline-none transition bg-transparent disabled:bg-gray-100" 
              ...
              <div className="flex items-center justify-end">
                <input 
                  ...
                  onChange={(e) => setData(prev => ({ ...prev, isEndDateIndefinite: e.target.checked, projectEndDate: e.target.checked ? '' : prev.projectEndDate }))} 
        ...
        return (
          <div className="text-center">
            ...
            <div className="flex justify-center border-b mb-6">
              <button onClick={() => setActiveTab('official')} className={`px-4 py-2 text-sm font-semibold flex items-center gap-2 ${activeTab === 'official' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500'}`}>
                <Star className="w-4 h-4"/>Modèles Officiels
              </button>
              <button onClick={() => setActiveTab('community')} className={`px-4 py-2 text-sm font-semibold flex items-center gap-2 ${activeTab === 'community' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500'}`}>
                <Users className="w-4 h-4"/>Communauté
              </button>
              <button onClick={() => setActiveTab('my-templates')} className={`px-4 py-2 text-sm font-semibold flex items-center gap-2 ${activeTab === 'my-templates' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500'}`}>
              ...
              {filteredTemplates.map(template => {
                ...
                return (
                  <button 
                    key={template.id} 
                    onClick={() => {
                      console.log("📋 OnboardingView: Template sélectionné", template.id);
        ...
        return (
          <div className="text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
              <button 
                onClick={() => { 
                  console.log("🎯 OnboardingView: Option 'avec données' sélectionnée");
                  setData(prev => ({...prev, startOption: 'populated'})); 
                  handleNext(); 
                }} 
              ...
              <button 
                onClick={() => { 
                  console.log("🎯 OnboardingView: Option 'vierge' sélectionnée");
                  setData(prev => ({...prev, startOption: 'blank'})); 
                  handleNext(); 
                }} 
  ...
  console.log("🎨 OnboardingView: Rendu final, step=", step, "isLoading=", isLoading);
